...
export default {  
  ...
  methods: {    
    init() {      
      this.selectedGridLength = 3;
      this.selectedPanelColor = '#00ff00';
      this.selectedPanelTransparency = false;
      this.panels = []; // Array for saving the panels
      this.isAddMode = true;
    ...
    addPanel(interactionManager, positionX, positionY, positionZ, direction) {
    ...
    onPanelOutlinesChange(event) {
      const checkbox = event.target;
      const showOutlines = checkbox.checked;
      updatePanelOutlines(showOutlines);
    },

    // Function to update the visibility of panel outlines
    updatePanelOutlines(showOutlinesan) {
      showOutlines = showOutlines;
      this.panels.forEach(panel => {
        const isPanelTransparent = (panel.material).opacity === 0;
        const outline = panel.getObjectByName('outline');
